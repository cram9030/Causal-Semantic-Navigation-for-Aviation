...
# `uv sync` installs the exact versions pinned in the committed uv.lock
# rather than re-resolving against whatever's newest on PyPI - see the
# Dockerfile's `uv` install step for why that matters for `ml` (torch).
uv sync --all-extras
